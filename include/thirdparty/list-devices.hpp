...
namespace v4l2
{
	namespace devices
	{
		...
		inline void list(std::vector<DEVICE_INFO> &devices)
		{
			...
			for (auto &row : device_map)
			{
				std::sort(row.second.device_paths.begin(), row.second.device_paths.end(), [](const std::string &a, const std::string &b){
					const char baseword[] = "/dev/video";
					const int offset = sizeof(baseword) -1;
					int anum = atoi(a.c_str() + offset);
					int bnum = atoi(b.c_str() + offset);
					return anum<bnum;
				});
