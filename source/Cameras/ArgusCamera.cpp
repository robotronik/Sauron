...
bool ArgusCamera::StartFeed()
{
	...
	IBufferOutputStreamSettings *iEglStreamSettings = interface_cast<IBufferOutputStreamSettings>(streamSettings);
	...
	//iEglStreamSettings->setResolution(Size2D<uint32_t>(1920,1080));
	...
	cout << "CaptureStream created succesfully" << endl;
	//ConsumerThread::Init

	m_consumer = UniqueObj<FrameConsumer>(FrameConsumer::create(captureStream.get()));
	if (!m_consumer){
		//ORIGINATE_ERROR("Failed to create FrameConsumer");
		return false;
	}

	cout << "consumer created succesfully" << endl;

	//ConsumerThread::ThreadExecute start

	iEglOutputStream = interface_cast<IBufferOutputStream>(captureStream.get());
